...
namespace cat::jit {
    ...
    if (!expected) {
      llvm::handleAllErrors(
          expected.takeError(), [&](const llvm::ErrorInfoBase &e) {
            diag.error(Span{}, string("JIT init: ") + e.message())
                .emit_to(diag);
          });
      return;
    }
  ...
  void JIT::add_module(ir::IrEmitter &emitter) {
    ...
    auto tsm = llvm::orc::ThreadSafeModule(std::move(handle.mod),
                                           std::move(handle.ctx));
    if (auto err = engine->addIRModule(std::move(tsm))) {
      llvm::handleAllErrors(
          std::move(err), [&](const llvm::ErrorInfoBase &e) {
            diag.error(Span{},
                       string("JIT add module: ") + e.message())
                .emit_to(diag);
          });
    }
  }
  ...
  void *JIT::lookup(const string &name) {
    ...
    if (!sym) {
      llvm::handleAllErrors(
          sym.takeError(), [&](const llvm::ErrorInfoBase &e) {
