...
uint32_t intersect(uint32_t finger1, uint32_t finger2,
                   const vector<uint32_t> &idoms,
                   const vector<uint32_t> &po_idx);
...
///
/// In the set of strict dominators, find the node that dominates all other
/// strict dominators That is: idom(n) = the "largest" element in the set of
/// strict dominators 快速计算直接支配者（基于 RPO + intersect）
vector<uint32_t> compute_idoms_fast(const CFG &cfg);

// build dominator from immediate dominator
vector<std::set<uint32_t>>
compute_dominators_fast(const CFG &cfg, const vector<uint32_t> &idoms);

vector<vector<uint32_t>>
compute_dom_tree_children(const vector<uint32_t> &idoms);

inline vector<std::set<uint32_t>>
compute_dominance_frontier(const CFG &cfg, const vector<uint32_t> &idom) {
  auto n = cfg.size();
