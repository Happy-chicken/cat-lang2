      ...
      while (!worklist.empty()) {
        ...
        switch (analysis.direction()) {
          case Direction::Forward: {
            ...
            State new_in = (id == entry) ? boundary : initial_or_meet(preds, out_states);
          ...
          case Direction::Backward: {
            ...
            State new_out = (id == exit) ? boundary : initial_or_meet(succs, in_states);
...
private:
    State initial_or_meet(const vector<uint32_t> &indices, const vector<State> &states) const {
      if (indices.empty()) return analysis.initial_state();
