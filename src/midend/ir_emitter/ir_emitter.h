...
namespace cat::ir {
  ...
  class IrEmitter {
public:
    IrEmitter(const string &name, error::DiagCtxt &diag, semantics::SemaCtxt &sema_ctx);
    ...
    void compile_class_constructor(const string &name, llvm::StructType *struct_ty, const ClassInfo &cls_info);
    void compile_global_var(const GlobalVar &gv, Span span);
    void compile_method(const string &cls_name, const FunctionDef &func, Span span);
    void compile_function(const FunctionDef &func, Span span);
    void compile_named_function(const FunctionDef &func, const string &name, Span span);
    ...
    llvm::Value *emit_list_literal(const vector<uptr<ExprNode>> &elements, Span span);
    void emit_list_with_init(llvm::Value *list_alloca, const vector<uptr<ExprNode>> &elements, Span span);
    void emit_list_with_init_fields(llvm::Value *list_alloca, const vector<uptr<ExprNode>> &elements, llvm::Type *elem_ty, Span span);
