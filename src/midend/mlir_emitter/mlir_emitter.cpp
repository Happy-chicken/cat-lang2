...
namespace {
inline mlir::Type i32_ty(mlir::MLIRContext &c) {
  return mlir::IntegerType::get(&c, 32);
}
inline mlir::Type f32_ty(mlir::MLIRContext &c) {
  return mlir::Float32Type::get(&c);
}
inline mlir::Type i1_ty(mlir::MLIRContext &c) {
  return mlir::IntegerType::get(&c, 1);
}
inline mlir::Type i8_ty(mlir::MLIRContext &c) {
  return mlir::IntegerType::get(&c, 8);
}
...
MlirEmitter::MlirEmitter(const string & /*name*/, error::DiagCtxt &diag,
                         semantics::SemaCtxt &sema_ctx)
    : ctx(std::make_unique<CodegenCtxt>()), env(std::make_shared<Env>()),
      diag(diag), sema(sema_ctx) {}
...
mlir::Type MlirEmitter::mlir_type(const ast::Type &ast_type) {
  return std::visit(
      overload{
          ...
          [&](const ast::Type::Struct &) { return i32_ty(ctx->mlir_ctx); },
          [&](const ast::Type::TraitObject &) { return i32_ty(ctx->mlir_ctx); },
...
void MlirEmitter::compile_function(const FunctionDef &func, Span span) {
  ...
  for (size_t i = 0; i < hdr.params.size(); ++i) {
    ...
    auto alloc =
        ctx->builder->create<mlir::memref::AllocaOp>(loc, memref_ty(elem_ty));
    ctx->builder->create<mlir::memref::StoreOp>(loc, arg, alloc.getResult(),
                                                mlir::ValueRange{});
  ...
  std::visit(
      overload{
          [&](const VarDefStmt &s) {
            ...
            auto alloc = ctx->builder->create<mlir::memref::AllocaOp>(
                loc, memref_ty(elem_ty));
            auto alloca = alloc.getResult();

            if (init_val) {
              ctx->builder->create<mlir::memref::StoreOp>(loc, init_val, alloca,
                                                          mlir::ValueRange{});
            } else {
              auto zero = ctx->builder->create<mlir::arith::ConstantOp>(
                  loc, elem_ty, ctx->builder->getIntegerAttr(elem_ty, 0));
              ctx->builder->create<mlir::memref::StoreOp>(
                  loc, zero.getResult(), alloca, mlir::ValueRange{});
            }
...
void MlirEmitter::compile_if(const IfStmt &s) {
  ...
  region->getBlocks().insertAfter(ctx->builder->getBlock()->getIterator(),
                                  then_block);
  if (s.else_branch)
    region->getBlocks().insertAfter(then_block->getIterator(), else_block);
  region->getBlocks().insertAfter(s.else_branch ? else_block->getIterator()
                                                : then_block->getIterator(),
                                  merge_block);

  ctx->builder->create<mlir::cf::CondBranchOp>(loc, cond, then_block,
                                               else_block);
...
void MlirEmitter::compile_while(const LoopStmt &s) {
  ...
  auto cond = compile_expr(s.condition);
  ctx->builder->create<mlir::cf::CondBranchOp>(loc, cond, body_block,
                                               exit_block);
  ...
  return std::visit(
      overload{
            ...
            return std::visit(
                overload{
                    ...
                    [&](bool v) -> mlir::Value {
                      ...
                      return ctx->builder
                          ->create<mlir::arith::ConstantOp>(
                              loc, ty, ctx->builder->getBoolAttr(v))
                          .getResult();
                    },
                    [&](float v) -> mlir::Value {
                      ...
                      return ctx->builder
                          ->create<mlir::arith::ConstantOp>(loc, ty, val)
                          .getResult();
                    },
                    [&](char v) -> mlir::Value {
                      ...
                      return ctx->builder
                          ->create<mlir::arith::ConstantOp>(
                              loc, ty,
                              ctx->builder->getIntegerAttr(
                                  ty, static_cast<int64_t>(v)))
                          .getResult();
                    },
                    [&](const std::string &) -> mlir::Value { return {}; },
                },
                e.lit);
          },
          [&](const Variable &e) -> mlir::Value {
            ...
            auto load = ctx->builder->create<mlir::memref::LoadOp>(
                loc, vi.memref, mlir::ValueRange{});
          ...
          [&](const AssignExpr &e) -> mlir::Value {
            ...
            std::visit(overload{
                           [&](const Variable &t) {
                             ...
                             if (vi.memref)
                               ctx->builder->create<mlir::memref::StoreOp>(
                                   loc, val, vi.memref, mlir::ValueRange{});
                           },
                           [](const auto &) {},
                       },
                       e.target->expr);
          ...
          [&](const BinaryExpr &e) -> mlir::Value {
            ...
            switch (e.op) {
            case BinaryOp::Add:
              return is_float ? ctx->builder
                                    ->create<mlir::arith::AddFOp>(loc, lhs, rhs)
                                    .getResult()
                              : ctx->builder
                                    ->create<mlir::arith::AddIOp>(loc, lhs, rhs)
                                    .getResult();
            case BinaryOp::Sub:
              return is_float ? ctx->builder
                                    ->create<mlir::arith::SubFOp>(loc, lhs, rhs)
                                    .getResult()
                              : ctx->builder
                                    ->create<mlir::arith::SubIOp>(loc, lhs, rhs)
                                    .getResult();
            case BinaryOp::Mul:
              return is_float ? ctx->builder
                                    ...
                                    .getResult()
                              : ctx->builder
                                    ->create<mlir::arith::MulIOp>(loc, lhs, rhs)
                                    .getResult();
            case BinaryOp::Div:
              return is_float
                         ? ctx->builder
                               ->create<mlir::arith::DivFOp>(loc, lhs, rhs)
                               .getResult()
                         : ctx->builder
                               ->create<mlir::arith::DivSIOp>(loc, lhs, rhs)
                               .getResult();
            case BinaryOp::Eq:
              return is_float ? ctx->builder
          ...
          [&](const UnaryExpr &e) -> mlir::Value {
            ...
            case UnaryOp::Neg:
              if (val.getType().isF32())
                return ctx->builder->create<mlir::arith::NegFOp>(loc, val)
                    .getResult();
              else {
                auto ty = val.getType();
                auto zero = ctx->builder->create<mlir::arith::ConstantOp>(
                    loc, ty, ctx->builder->getIntegerAttr(ty, 0));
                return ctx->builder
                    ->create<mlir::arith::SubIOp>(loc, zero.getResult(), val)
                    .getResult();
              }
            case UnaryOp::Not: {
              auto ty = val.getType();
                    ...
                    [&](const Variable &callee) -> mlir::Value {
                      ...
                      auto callee_fn =
                          mlir::SymbolTable::lookupNearestSymbolFrom<
                              mlir::func::FuncOp>(
                              ctx->module, mlir::StringAttr::get(&ctx->mlir_ctx,
                                                                 callee.name));
                      if (callee_fn) {
                        result_tys = callee_fn.getFunctionType().getResults();
                      }

                      auto call = ctx->builder->create<mlir::func::CallOp>(
                          loc, callee.name, result_tys, args);
                      if (result_tys.empty())
                        return mlir::Value{};
          ...
          [&](const ListExpr &) -> mlir::Value { return {}; },
          [&](const MemberExpr &) -> mlir::Value { return {}; },
          [&](const IndexExpr &) -> mlir::Value { return {}; },
          [&](const LambdaExpr &) -> mlir::Value { return {}; },
      },
      expr);
}
