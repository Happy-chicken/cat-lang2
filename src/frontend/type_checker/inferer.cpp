...
namespace cat::semantics {
  ...
  Type Inferer::ast_type_to_semantic_type(const ast::Type &ast_type) {
    return std::visit(
        overloaded{
            ...
            [](const ast::Type::Func &func) -> Type {
              std::vector<uptr<Type>> param_types;
              param_types.reserve(func.params.size());
              for (const auto &param: func.params) {
                if (param) {
                  param_types.push_back(
                      std::make_unique<Type>(ast_type_to_semantic_type(*param))
                  );
                } else {
              ...
              Type ret_type = func.ret
                                  ? ast_type_to_semantic_type(*func.ret)
  ...
  Type Inferer::infer_expr(const Expr &expr, Span span, SemaCtxt &ctxt, error::DiagCtxt &diag) {
    return std::visit(
        overloaded{
  ...
  Type Inferer::infer_let_binding(const optional<Type> &declared_type, const optional<ExprNode> &init_expr, Span span, SemaCtxt &ctxt, error::DiagCtxt &diag) {
    auto inferred_type =
  ...
  Type Inferer::infer_assignment(const AssignExpr &assign, Span span, SemaCtxt &ctx, error::DiagCtxt &diag) {
                  ...
                  overloaded{
                      ...
                      [&](const error::InfiniteType &infinite) {
                        diag.error(span, "Infinite type in assignment")
                            .note("Type variable " + std::to_string(infinite.var) + " appears recursively")
                            .emit_to(diag);
                      },
                  },
                  err
  ...
  Type Inferer::infer_binary(const BinaryExpr &bin, Span span, SemaCtxt &ctxt, error::DiagCtxt &diag) {
  ...
  Type Inferer::infer_unary(const UnaryExpr &unary, Span span, SemaCtxt &ctxt, error::DiagCtxt &diag) {
  ...
  Type Inferer::infer_call(const CallExpr &call, Span span, SemaCtxt &ctxt, error::DiagCtxt &diag) {
    ...
    if (auto *func = std::get_if<Type::Func>(&resolved.get_data())) {
      ...
      if (expected_params != arg_types.size()) {
        diag.error(span, "Argument count mismatch: expected " + std::to_string(expected_params) + ", got " + std::to_string(arg_types.size()))
            .emit_to(diag);
        return Type::error();
      }
      ...
      for (size_t i = 0; i < arg_types.size(); ++i) {
        const auto &expected = *func->params[i + param_offset];
  ...
  Type Inferer::infer_member(const MemberExpr &member, Span span, SemaCtxt &ctxt, error::DiagCtxt &diag) {
    Type obj_ty =
  ...
  Type Inferer::infer_list_literal(const ListExpr &list, Span span, SemaCtxt &ctxt, error::DiagCtxt &diag) {
    ...
    for (size_t i = 1; i < list.elements.size(); ++i) {
      Type ty = infer_expr(list.elements[i]->expr, list.elements[i]->span, ctxt, diag);
      auto result = unifier.unify(elem_ty, ty);
      if (std::holds_alternative<error::UnifyError>(result)) {
        diag.error(span, "List element type mismatch at index " + std::to_string(i))
