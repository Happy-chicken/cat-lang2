  ...
  return std::visit(
      overloaded{
          ...
          [&](const Type::Func &func) -> optional<ast::Type> {
            ...
            return ast::Type(
                ast::Type::Func{std::move(param_types),
                                std::make_unique<ast::Type>(std::move(*ret))});
          },
      ...
      overloaded{
          [&](const VarDefStmt &var_def) {
            ...
            if (var_def.ty.has_value()) {
              declared_type = Inferer::ast_type_to_semantic_type(*var_def.ty);
            }
            Type inferred = inferer.infer_let_binding(
                declared_type, var_def.init, span, ctx, diag);
          ...
          [&](const IfStmt &if_stmt) {
            Type cond_ty = inferer.infer_expr(
                if_stmt.condition.expr, if_stmt.condition.span, ctx, diag);
            check_condition_is_bool(cond_ty, if_stmt.condition.span, ctx, diag);
            check_block(*if_stmt.then_branch, span, ctx, diag);

            for (const auto &[elif_cond, elif_block] : if_stmt.elif_branch) {
              Type elif_ty =
          ...
          [&](const ReturnStmt &ret) {
            if (ret.expr.has_value()) {
              Type ret_ty =
                  inferer.infer_expr(ret.expr->expr, ret.expr->span, ctx, diag);
              if (current_return_type.has_value()) {
                Unifier unifier(ctx.get_type_ctxt());
                auto result = unifier.unify(ret_ty, *current_return_type);
...
void TypeChecker::check_class_defaults(const Class &cls, SemaCtxt &ctx,
  ...
  for (const auto &field : cls.fields) {
    if (field.init.has_value()) {
      ...
      Type init_ty =
          inferer.infer_expr(field.init->expr, field.init->span, ctx, diag);

      Unifier unifier(ctx.get_type_ctxt());
      auto result = unifier.unify(init_ty, decl_ty);
      if (std::holds_alternative<error::UnifyError>(result)) {
        diag.error(field.init->span,
                   "Field default value type mismatch for '" + field.name + "'")
