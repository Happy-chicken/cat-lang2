  ...
  return std::visit(
      overloaded{
          [](const Type::Prim &prim) -> optional<ast::Type> {
            switch (prim.kind) {
            case PrimType::Int:   return ast::type_int();
            case PrimType::Float: return ast::type_float();
            case PrimType::Bool:  return ast::type_bool();
            case PrimType::Char:  return ast::type_char();
            case PrimType::Str:   return ast::type_str();
            case PrimType::Void:  return ast::type_void();
            }
            return std::nullopt;
          },
...
void TypeChecker::check_function(const FunctionDef &func, Span span,
  ...
  for (const auto &param : func.function_header.params) {
    Symbol param_sym =
        Symbol::new_parameter(param.name, param.ty.clone(), param.is_ref, span);
...
void TypeChecker::check_stmt(const StmtNode &stmt_node, Span span,
                             SemaCtxt &ctx, error::DiagCtxt &diag) {
  std::visit(
      overloaded{
          [&](const VarDefStmt &var_def) {
            ...
            Type inferred = inferer.infer_let_binding(declared_type,
                                                          var_def.init, span,
                                                          ctx, diag);
          ...
          [&](const IfStmt &if_stmt) {
            Type cond_ty = inferer.infer_expr(if_stmt.condition.expr,
                                              if_stmt.condition.span, ctx, diag);
          ...
          [&](const LoopStmt &loop) {
            Type cond_ty = inferer.infer_expr(loop.condition.expr,
                                              loop.condition.span, ctx, diag);
            check_condition_is_bool(cond_ty, loop.condition.span, ctx, diag);
            check_block(*loop.body, span, ctx, diag);
          },
          [&](const ExprStmt &expr_stmt) {
            inferer.infer_expr(expr_stmt.expr.expr, expr_stmt.expr.span, ctx,
                               diag);
          },
          [&](const ReturnStmt &ret) {
            if (ret.expr.has_value()) {
              Type ret_ty = inferer.infer_expr(ret.expr->expr,
                                               ret.expr->span, ctx, diag);
...
void TypeChecker::check_block(const Block &block, Span span, SemaCtxt &ctx,
                              error::DiagCtxt &diag) {
...
void TypeChecker::check_condition_is_bool(const Type &ty, Span span,
                                          SemaCtxt &ctx,
                                          error::DiagCtxt &diag) {
...
void TypeChecker::check_global_var(const GlobalVar &gv, Span span,
                                   SemaCtxt &ctx, error::DiagCtxt &diag) {
  optional<Type> declared_type;
  if (gv.ty.has_value()) {
...
void TypeChecker::check_class_defaults(const Class &cls, SemaCtxt &ctx,
  ...
  for (const auto &field : cls.fields) {
    if (field.init.has_value()) {
      ...
      Type init_ty = inferer.infer_expr(field.init->expr, field.init->span,
                                        ctx, diag);

      Unifier unifier(ctx.get_type_ctxt());
      auto result = unifier.unify(init_ty, decl_ty);
      if (std::holds_alternative<error::UnifyError>(result)) {
        diag.error(field.init->span,
                   "Field default value type mismatch for '" +
                       field.name + "'")
