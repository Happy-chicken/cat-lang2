...
namespace cat::semantics {
  ...
  error::UnifyResult<semantics::Type> Unifier::unify(const Type &t1, const Type &t2) {
        ...
        overloaded{
            ...
            [&](const Type::Func &fa, const Type::Func &fb)
              ...
              if (std::holds_alternative<error::UnifyError>(ret))
                return error::UnifyError{error::Mismatch{a.clone(), b.clone()}};
              return Type::func(std::move(unified_params), std::move(std::get<Type>(ret)));
            },
  ...
  error::UnifyResult<semantics::Type> Unifier::unify_var(TypedVar var, const Type &ty) {
