...
error::UnifyResult<semantics::Type> Unifier::unify(const Type &t1,
  ...
  return std::visit(
      overloaded{
          [&](const Type::Prim &pa,
              const Type::Prim &pb) -> error::UnifyResult<semantics::Type> {
            if (pa.kind == pb.kind)
              return a.clone();
          ...
          [&](const Type::Ref &ra,
              const Type::Ref &rb) -> error::UnifyResult<semantics::Type> {
          ...
          [&](const Type::Own &oa,
              const Type::Own &ob) -> error::UnifyResult<semantics::Type> {
          ...
          [&](const Type::Func &fa,
              const Type::Func &fb) -> error::UnifyResult<semantics::Type> {
            ...
            for (size_t i = 0; i < fa.params.size(); ++i) {
              ...
              if (std::holds_alternative<error::UnifyError>(p))
                return error::UnifyError{error::Mismatch{a.clone(), b.clone()}};
              unified_params.push_back(
                  std::make_unique<Type>(std::move(std::get<Type>(p))));
            }
            if (!fa.ret || !fb.ret)
              return error::UnifyError{error::Mismatch{a.clone(), b.clone()}};
