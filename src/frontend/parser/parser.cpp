...
error::ParseResult<StmtNode> Parser::parse_if_stmt() {
  ...
  while (check(TokenKind::Elif)) {
    ...
    elif_braches.push_back(
        std::make_pair(std::move(*elif_condition),
                       std::make_unique<Block>(std::move(*elif_block))));
  }
  ...
  case TokenKind::LeftParen: {
    ...
    if (!check(TokenKind::RightParen)) {
      ...
      param_types.push_back(
          std::make_unique<ast::Type>(std::move(*first_type)));
      while (check(TokenKind::Comma)) {
        ...
        param_types.push_back(
            std::make_unique<ast::Type>(std::move(*next_type)));
      }
    }
    ...
    return type_func(std::move(param_types),
                     std::make_unique<ast::Type>(std::move(*ret_type)));
  }
  default:
...
error::ParseResult<Field> Parser::parse_field() {
  consume(TokenKind::Let,
          "Expected 'let' at the beginning of field definition.");
  auto field_token =
      consume(TokenKind::Identifier, "Expected identifier after 'let'.");
  if (!field_token.has_value()) {
    return std::nullopt;
...
error::ParseResult<ExprNode> Parser::parse_lambda() {
  ...
  auto func_expr = make_lambda(std::move(params), std::move(return_type),
                               std::make_unique<Block>(std::move(*body)));
