...
namespace cat::ast {
  ...
  namespace {
    ...
    std::string type_to_string(const Type &type) {
      ...
      std::visit(overloaded{
                     ...
                     [&](const Type::Func &t) {
                       auto param_strings = t.params | std::views::transform([](const auto &p) {
                                              return p ? type_to_string(*p) : "?";
                                            });
                       ss << "(";
                       bool first = true;
    ...
    void print_params_inline(std::ostream &os, const std::vector<Parameter> &params) {
      os << "(";
    ...
    void print_expr_impl(std::ostream &os, const std::string &prefix, bool is_last, const Expr &expr);
    void print_stmt_impl(std::ostream &os, const std::string &prefix, bool is_last, const Stmt &stmt);
    void print_item_impl(std::ostream &os, const std::string &prefix, bool is_last, const Item &item);

    void print_expr_ptr(std::ostream &os, const std::string &prefix, bool is_last, const uptr<ExprNode> &ptr) {
      if (ptr) {
    ...
    void print_opt_expr_node(std::ostream &os, const std::string &prefix, bool is_last, const std::optional<ExprNode> &opt) {
      if (opt) {
      ...
      std::visit(overloaded{
                     ...
                     [&](const CallExpr &e) {
                       ...
                       for (size_t i = 0; i < e.args.size(); ++i) {
                         print_expr_ptr(os, child_prefix, i == e.args.size() - 1, e.args[i]);
                       }
                     },
                     [&](const MemberExpr &e) {
          ...
          overloaded{
              ...
              [&](const IfStmt &e) {
                ...
                for (size_t i = 0; i < e.elif_branch.size(); ++i) {
                  ...
                  print_expr_node(os, elif_cond_prefix, true, e.elif_branch[i].first);
  ...
  void print_expr_node(std::ostream &os, const std::string &prefix, bool is_last, const cat::ExprNode &node) {
    print_expr_impl(os, prefix, is_last, node.expr);
  }

  void print_stmt_node(std::ostream &os, const std::string &prefix, bool is_last, const cat::StmtNode &node) {
    print_stmt_impl(os, prefix, is_last, node.stmt);
  }

  void print_block(std::ostream &os, const std::string &prefix, bool is_last, const cat::Block &block) {
    os << prefix << branch(is_last) << "Block\n";
    auto child_prefix = next_prefix(prefix, is_last);
    for (size_t i = 0; i < block.stmts.size(); ++i) {
      print_stmt_node(os, child_prefix, i == block.stmts.size() - 1, block.stmts[i]);
    }
  }

  void print_item_node(std::ostream &os, const std::string &prefix, bool is_last, const cat::ItemNode &node) {
