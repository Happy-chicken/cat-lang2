...
std::string type_to_string(const Type &type) {
  ...
  std::visit(overloaded{
                 ...
                 [&](const Type::Func &t) {
                   auto param_strings =
                       t.params | std::views::transform([](const auto &p) {
...
void print_expr_impl(std::ostream &os, const std::string &prefix, bool is_last,
  ...
  std::visit(overloaded{
                 ...
                 [&](const LambdaExpr &e) {
                   ...
                   if (e.return_type) {
                     os << child_prefix << branch(!e.body) << "-> "
                        << type_to_string(*e.return_type) << "\n";
                   }
                   if (e.body) {
...
namespace cat {
LambdaExpr::LambdaExpr(vector<Parameter> p, optional<ast::Type> r,
                       uptr<Block> b)
...
Expr make_lambda(vector<Parameter> params, optional<ast::Type> return_type,
                 uptr<Block> body) {
  return Expr{
      LambdaExpr{std::move(params), std::move(return_type), std::move(body)}};
}
} // namespace cat
