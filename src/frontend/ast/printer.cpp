...
const char *to_string(BinaryOp op) {
  ...
  case BinaryOp::NotEq: return "NotEq";
...
std::string type_to_string(const Type &type) {
  ...
  std::visit(overloaded{
    ...
    [&](const Type::Ptr &t)  { ss << "Ptr<" << type_to_string(*t.inner) << ">"; },
    [&](const Type::List &t) { ss << "List<" << type_to_string(*t.inner) << ">"; },
...
void print_expr_ptr(std::ostream &os, const std::string &prefix, bool is_last,
                    const uptr<Expr> &ptr) {
  if (ptr) {
    print_expr_impl(os, prefix, is_last, *ptr);
  } else {
...
void print_expr_impl(std::ostream &os, const std::string &prefix, bool is_last,
  ...
  std::visit(overloaded{
    ...
    [&](const BinaryExpr &e) {
      os << prefix << branch(is_last) << "BinaryExpr: " << to_string(e.op) << "\n";
      auto child_prefix = next_prefix(prefix, is_last);
      print_expr_ptr(os, child_prefix, false, e.lhs);
      print_expr_ptr(os, child_prefix, true, e.rhs);
    },
    [&](const UnaryExpr &e) {
      os << prefix << branch(is_last) << "UnaryExpr: " << to_string(e.op) << "\n";
      auto child_prefix = next_prefix(prefix, is_last);
...
void print_stmt_impl(std::ostream &os, const std::string &prefix, bool is_last,
  ...
  std::visit(overloaded{
    [&](const VarDefStmt &e) {
      ...
      if (e.ty) os << " : " << type_to_string(*e.ty);
      os << "\n";
      if (e.init) {
        auto child_prefix = next_prefix(prefix, is_last);
...
void print_item_impl(std::ostream &os, const std::string &prefix, bool is_last,
  ...
  std::visit(overloaded{
    [&](const FunctionDef &e) {
      os << prefix << branch(is_last) << "FunctionDef: " << e.function_header.name << "\n";
      auto fn_prefix = next_prefix(prefix, is_last);

      os << fn_prefix << branch(false) << "Params: ";
      print_params_inline(os, e.function_header.params);
      os << "\n";

      bool has_return = e.function_header.return_type.has_value();
      os << fn_prefix << branch(!has_return) << "ReturnType: ";
      if (has_return) {
    ...
    [&](const FunctionDecl &e) {
      ...
      os << fn_prefix << branch(true) << "ReturnType: ";
      if (e.return_type) {
    ...
    [&](const Class &e) {
      ...
      for (size_t i = 0; i < e.fields.size(); ++i) {
        os << cls_prefix << branch(i == e.fields.size() - 1) << "Field: " << e.fields[i].name
           << " : " << type_to_string(e.fields[i].ty);
        if (e.fields[i].init) {
          os << " =\n";
          auto field_prefix = next_prefix(cls_prefix, i == e.fields.size() - 1);
    ...
    [&](const Trait &e) {
      ...
      for (size_t i = 0; i < e.methods.size(); ++i) {
        os << trait_prefix << branch(i == e.methods.size() - 1) << "Method: "
           << e.methods[i].name;
    ...
    [&](const Impl &e) {
      ...
      for (size_t i = 0; i < e.methods.size(); ++i) {
        ...
        auto meth_prefix = next_prefix(impl_prefix, i == e.methods.size() - 1);

        os << meth_prefix << branch(false) << "Params: ";
        print_params_inline(os, e.methods[i].function_header.params);
        os << "\n";

        bool has_return = e.methods[i].function_header.return_type.has_value();
        os << meth_prefix << branch(!has_return) << "ReturnType: ";
        if (has_return) {
          os << type_to_string(*e.methods[i].function_header.return_type) << "\n";
        } else {
          os << "(none)\n";
        }
