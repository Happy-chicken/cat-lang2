...
namespace cat {
  ...
  optional<char> Lexer::peek_next() const {
    ...
    if (next == src.end()) return std::nullopt;
    return *next;
  }
  ...
  std::string_view Lexer::current_token() const {
    return {token_start, it};
  }

  auto Lexer::chars() const {
    return std::views::all(src);
  }
  ...
  Token Lexer::number() {
    ...
    while (!is_eof()) {
      ...
      if (std::isdigit(static_cast<unsigned char>(c))) {
        ...
        auto next = peek_next();
        if (!next.has_value() || !std::isdigit(static_cast<unsigned char>(next.value()))) {
          break;
        }
        if (is_float) break;
        is_float = true;
    ...
    return make_token(
        is_float ? TokenKind::FloatLiteral : TokenKind::IntLiteral,
  ...
  Token Lexer::string() {
    ...
    return make_token(TokenKind::StringLiteral, std::string(current_token()));
  }
  ...
  Token Lexer::character() {
    ...
    return make_token(TokenKind::CharLiteral, std::string(current_token()));
  }
