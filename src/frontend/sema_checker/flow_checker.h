...
class FlowChecker : public semantics::Pass {
  ...
  const char *name() const noexcept override { return "FlowChecker"; }
          ...
          overloaded{[&](const FunctionDef &func) {
                     ...
                     [&](const Impl &imp) {
                       for (const auto &method : imp.methods) {
                         ast::Type return_type =
                             method.function_header.return_type
                                 ? method.function_header.return_type->clone()
                                 : ast::type_void();
  ...
  void check_function_returns(const FunctionDef &func,
