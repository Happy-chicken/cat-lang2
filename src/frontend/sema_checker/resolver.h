...
namespace cat {
  class Resolver : public semantics::Pass {
    ...
    void declare_impl_methods(const ItemNode &item_node, semantics::SemaCtxt &ctx, error::DiagCtxt &diag);

    void declare_global_var(const ItemNode &item_node, semantics::SemaCtxt &ctx, error::DiagCtxt &diag);
