...
public:
  ...
  static Symbol new_variable(string name, optional<ast::Type> ty,
                             bool is_mutable, Span span, BorrowKind borr_kind = BorrowKind::None,
  ...
  static Symbol new_parameter(string name, ast::Type ty, BorrowKind borr_kind,
                              Span span);
  ...
  bool is_variable() const {
    return std::holds_alternative<VariableData>(kind);
  }

  bool is_function() const {
    return std::holds_alternative<FunctionData>(kind);
  }

  bool is_ref() const {
    if (auto *param = std::get_if<ParameterData>(&kind)) {
      return param->borr_kind == BorrowKind::Ref;
    }
    if (auto *var = std::get_if<VariableData>(&kind)) {
      return var->borr_kind == BorrowKind::Ref;
    }
    return false;
  }
  ...
  bool is_cref() const {
    if (auto *param = std::get_if<ParameterData>(&kind)) {
      return param->borr_kind == BorrowKind::CRef;
    }
    if (auto *var = std::get_if<VariableData>(&kind)) {
      return var->borr_kind == BorrowKind::CRef;
    }
    return false;
  }
  ...
  bool is_own() const {
    if (auto *param = std::get_if<ParameterData>(&kind)) {
      return param->borr_kind == BorrowKind::Own;
    }
    if (auto *var = std::get_if<VariableData>(&kind)) {
      return var->borr_kind == BorrowKind::Own;
    }
    return false;
  }
