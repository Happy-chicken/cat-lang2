...
find_duplicate_field(const vector<cat::Field> &fields) {
...
static optional<std::string>
find_duplicate_param(const vector<cat::Parameter> &params) {
  std::unordered_set<std::string> seen;
    ...
    std::visit(
        overloaded{
            ...
            [&](const FunctionDef &fn) {
              declare_value_item(item_node, ctx, diag);
            },
            [&](const GlobalVar &gv) {
              declare_global_var(gv, item_node.span, ctx, diag);
            },
            [](const auto &) {}},
        item_node.item);
  }
...
void Resolver::declare_top_level(Symbol symbol, semantics::SemaCtxt &ctx,
                                 error::DiagCtxt &diag) {
  ...
  if (existing) {
    ...
    diag.error(span, "the name `" + name + "` (" + kind_name +
...
void Resolver::declare_trait_item(const ItemNode &item_node,
                                  semantics::SemaCtxt &ctx,
                                  error::DiagCtxt &diag) {
  std::visit(
      overloaded{[&](const Trait &tr) {
                   ...
                   Symbol sym = Symbol::new_trait(
                       tr.name, std::move(method_name_list), item_node.span);
                   declare_top_level(std::move(sym), ctx, diag);
                 },
                 [](const auto &) {}},
      item_node.item);
...
void Resolver::declare_value_item(const ItemNode &item_node,
                                  semantics::SemaCtxt &ctx,
                                  error::DiagCtxt &diag) {
  string name;
  ...
  bool found = std::visit(
      overloaded{[&](const FunctionDecl &func) -> bool {
                   name = func.name;
                   params_ptr = &func.params;
                   return_type =
                       func.return_type
                           ? optional<ast::Type>(func.return_type->clone())
                           : std::nullopt;
                   return true;
                 },
                 [&](const FunctionDef &func) -> bool {
                   ...
                   return_type =
                       func.function_header.return_type
                           ? optional<ast::Type>(
                                 func.function_header.return_type->clone())
                           : std::nullopt;
                   return true;
                 },
  ...
  Symbol sym = Symbol::new_function(name, std::move(param_types),
                                    std::move(ret_ty), item_node.span);
  declare_top_level(std::move(sym), ctx, diag);
}

void Resolver::declare_impl_methods(const Impl &impl, Span span,
                                    semantics::SemaCtxt &ctx,
                                    error::DiagCtxt &diag) {
  for (const auto &method : impl.methods) {
    ...
    if (header.params.empty()) {
      diag.error(span, "Method '" + header.name +
      ...
      if (first.name != "self") {
        diag.error(span,
                   "Method '" + header.name +
                       "' must have 'self' as its first parameter, found '" +
                       first.name + "'")
            .emit_to(diag);
      } else {
...
void Resolver::declare_global_var(const GlobalVar &gv, Span span,
                                  semantics::SemaCtxt &ctx,
                                  error::DiagCtxt &diag) {
...
void Resolver::detect_cycle(
    ...
    std::unordered_set<string> &visited, vector<string> &stack,
    std::unordered_set<string> &in_stack, error::DiagCtxt &diag) {
...
void Resolver::check_struct_recursion(const Program &program,
                                      error::DiagCtxt &diag) {
