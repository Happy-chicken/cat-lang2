...
namespace cat {
            ...
            overloaded{
                [&](const TraitData &trait) {
                  for (const auto &required: trait.methods) {
                    auto found = std::find_if(imp.methods.begin(), imp.methods.end(), [&](const FunctionDef &method) {
  ...
  void SemaChecker::check_expr(const ExprNode &expr, Span span, semantics::SemaCtxt &ctx, error::DiagCtxt &diag) {
    std::visit(overloaded{
                   ...
                   [&](const LiteralExpr &lit) {}, [&](const Variable &var) {
        ...
        check_expr(*assign.value, assign.value->span, ctx, diag); }, [&](const BinaryExpr &binary) {
        check_expr(*binary.lhs, binary.lhs->span, ctx, diag);
        check_expr(*binary.rhs, binary.rhs->span, ctx, diag); }, [&](const UnaryExpr &unary) {
        ...
        } }, [&](const MemberExpr &member) { check_expr(*member.object, member.object->span, ctx, diag); }, [&](const IndexExpr &index) {
