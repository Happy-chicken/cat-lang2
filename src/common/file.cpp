...
namespace cat {
	...
	const Location File::lookup_loc(Span::BytePos pos) const{
		...
		size_t line_idx = (it == line_starts.begin()) ? 0 : (it - line_starts.begin() - 1);
	...
	optional<std::string_view> File::get_line(size_t line_1based) const{
		if (line_1based == 0) return std::nullopt;
			
		size_t idx = line_1based - 1;
		if (idx >= line_starts.size()) return std::nullopt;

		size_t start = line_starts[idx];
		size_t end = (idx + 1 < line_starts.size()) ? line_starts[idx + 1] : content.size();
